...
async requestPairingCode(phoneNumber) {
    ...
    try {
        ...
        try {
            // Solicităm direct codul de asociere de la socket
            ...
            // Verificăm să nu fie codul demo
            if (code === 'ERROR-DEMO-CODE') {
                console.warn('WARNING: Received demo code 12345678');
            }
            
            this.emit('pairing_code', code);
            return code;
        } catch (socketError) {
            console.log('Error from main socket:', socketError.message);
            
            // Dacă metoda principală eșuează, încercăm metoda alternativă cu un socket nou
            if (socketError.message.includes('scan the QR code first') || 
                socketError.message.includes('Precondition Required')) {
                console.log('Trying alternative method - creating new socket...');
                
                // Creăm o sesiune separată pentru codul de asociere
                ...
                console.log('Requesting pairing code from alternative socket...');
                try {
                    const altCode = await pairingSocket.requestPairingCode(formattedNumber);
                    
                    console.log('Alternative pairing code response:', typeof altCode, altCode);
                    
                    // Different versions of baileys might return different formats
                    let finalCode;
                    if (typeof altCode === 'string') {
                        finalCode = altCode;
                    } else if (altCode && altCode.code) {
                        finalCode = altCode.code;
                    } else if (altCode && typeof altCode.toString === 'function') {
                        finalCode = altCode.toString();
                    } else {
                        throw new Error('Invalid response format from alternative socket');
                    }
                    
                    // Închide socket-ul temporar după ce am obținut codul
                    setTimeout(() => {
                        try {
                            pairingSocket.end();
                        } catch (e) {
                    ...
                    this.emit('pairing_code', finalCode);
                    return finalCode;
                } catch (altError) {
                    console.error('Alternative socket also failed:', altError.message);
                    throw altError; // Propagăm eroarea dacă și metoda alternativă eșuează
                }
            } else {
                // Dacă eroarea nu este legată de QR, o propagăm direct
                throw socketError;
            }
        }
    } catch (error) {
